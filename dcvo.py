...
#                pred_disp_rawscale = [tf.image.resize_bilinear(pred_disp[i], [opt.img_height, opt.img_width]) for i in range(opt.num_scales)]
#                if opt.with_pose:
#                    pred_depth_rawscale = [1. / (0.01+preddisp) for preddisp in pred_disp_rawscale]
#                    pred_depth =  [1. / (0.01+preddisp) for preddisp in pred_disp[:opt.num_scales+1]]    # -with_pose-2
#                else:
#                    pred_depth_rawscale = disp_to_depth(pred_disp_rawscale, 0.1, 100)
#                    pred_depth =  [disp_to_depth(pred_disp[i], 0.1, 100) for i in range(opt.num_scales)]    # -with_pose-2
#
#                #pred_depth = pred_depth_rawscale
#                #pred_disp = pred_disp_rawscale
#                # pred_depth =  [disp_to_depth(pred_disp[i], 0.1, 100) for i in range(opt.num_scales)]    # -with_pose
...
#
#                #tgt_image_pyramid = [tf.image.resize_nearest_neighbor(tgt_image, [np.int(opt.img_height // (2 ** s)), np.int(opt.img_width // (2 ** s))]) for s in range(opt.num_scales)]
#
#                # src depths
...
#                    src_pred_disp_rawscale = [tf.image.resize_bilinear(src_pred_disp[i], [opt.img_height, opt.img_width]) for i in range(opt.num_scales)]
#                    if opt.with_pose:
#                        src_pred_depth_rawscale = [1. / (0.01+preddisp) for preddisp in src_pred_disp_rawscale]
#                        src_pred_depth =  [1. / (0.01+preddisp) for preddisp in src_pred_disp[:opt.num_scales+1]]    # -with_pose-2
#                    else:
#                        src_pred_depth_rawscale = disp_to_depth(src_pred_disp_rawscale, 0.1, 100)
#                        src_pred_depth =  [disp_to_depth(src_pred_disp[i], 0.1, 100) for i in range(opt.num_scales)]    # -with_pose-2
#
#                    #src_pred_depth = src_pred_depth_rawscale
#                    #src_pred_disp = src_pred_disp_rawscale
#                    # src_pred_depth =  [disp_to_depth(src_pred_disp[i], 0.1, 100) for i in range(opt.num_scales)]     #-with_pose
...
#                                print (" > [batch%.2d] positionErr[%d] = %.6f" % (b,n,PE[n]))
    ...
    def build_pose_test_graph_dcvo(self, input_uint8, input_flow, input_depth, input_seglabel):
        """
    ...
    def setup_inference(self, 
        ...
        if self.mode == 'dcvo':
            self.seq_length = seq_length
            self.num_source = seq_length - 1
            self.build_pose_test_graph_dcvo(input_img_uint8, input_flow, input_depth, input_seglabel)
