...
def flow_to_image(flow):
    ...
    :param flow: optical flow map. shape=[B, h, w, 2]
    ...
    u = tf.where(tf.abs(u) > UNKNOWN_FLOW_THRESH, tf.zeros_like(u), u)
    v = tf.where(tf.abs(v) > UNKNOWN_FLOW_THRESH, tf.zeros_like(v), v)
    ...
    img = tf.where(tf.abs(tf.stack([u,u,u], -1)) > UNKNOWN_FLOW_THRESH, tf.zeros_like(img), img)
    img = tf.where(tf.abs(tf.stack([v,v,v], -1)) > UNKNOWN_FLOW_THRESH, tf.zeros_like(img), img)
...
def compute_color(u, v):
    ...
    :param u: optical flow horizontal map. shape=[B, h, w]
    :param v: optical flow vertical map. shape=[B, h, w]
    :return: optical flow in color code
    """
    B, h, w = u.shape.as_list()

    colorwheel = make_color_wheel()
    ncols, channel = colorwheel.shape

    rad = tf.sqrt(u**2+v**2)

    a = tf.atan2(-v, -u) / np.pi
    ...
    k1 = k0 + 1
    k1 = tf.where(k1 == ncols+1, tf.ones_like(k1), k1)
    f = fk - k0

    img = []
    for i in range(0, channel):
        tmp = colorwheel[:, i]
        col0 = tf.gather(tmp, tf.cast(k0-1, tf.int32)) / 255.
        col1 = tf.gather(tmp, tf.cast(k1-1, tf.int32)) / 255.
        col0 = tf.cast(col0, tf.float32)
        col1 = tf.cast(col0, tf.float32)
        col = (1-f) * col0 + f * col1   # shpae=[B,h,w]

        col = tf.where(rad <= 1, 1-rad*(1-col), col*0.75)
        img.append(tf.floor(255. * col))

    img = tf.stack(img, -1)
    print ("flow-color:", img)
