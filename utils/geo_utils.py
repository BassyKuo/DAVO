...
def meshgrid(batch, height, width, is_homogeneous=True, swap=False):
    ...
    if is_homogeneous:
        ...
        if swap:
            coords = tf.stack([y_t, x_t], axis=0)
        else:
...
def get_inv_pose(pose_vec):
    """[summary]
    get inverse pose
    Arguments:
        _pose_vec {[batch_size, 6]} -- [rz, ry, rx, tx, ty, tz]
        Note: this is different from the original SfMLearner which is [tx, ty, tz, rx, ry, rz].
    Returns:
        inverse pose
    """
    pose_mat = pose_vec2mat(pose_vec)
    pose_mat_inv = tf.linalg.inv(pose_mat)

    _rot = tf.slice(pose_mat_inv, [0, 0, 0], [-1, 3, 3])
    _rot_vec = mat2euler(_rot)
    _trans_vec = tf.slice(pose_mat_inv, [0, 0, 3], [-1, 3, 1])
    pose_vec_inv = tf.squeeze(tf.concat([_rot_vec, _trans_vec], axis=1))

    return pose_vec_inv

def warp_depth_by_flow(src_depth, flow):
    """Warp a source depth to the target depth plane based on optical flow movement.

    Args:
      src_depth: the source depth [batch, height_s, width_s, 1]
      flow: the target-to-source optical flow [batch, height_s, width_s, 2]
    Returns:
      Source depth warped to the target depth plane [batch, height_t, width_t, 1]
    """
    batch, height, width, _ = src_depth.get_shape().as_list()

    # Construct pixel grid coordinates
    pixel_coords = meshgrid(batch, height, width, is_homogeneous=False, swap=True) #shape=[B, 2, h, w]. coor=(row,col)=(y,x)
    pixel_coords = tf.transpose(pixel_coords, [0, 2, 3, 1]) #shape=[B,h,w,2]
    flow_h, flow_v = tf.split(flow, [1, 1], axis=3)
    flow = tf.concat([flow_v, flow_h], axis=3) #coor=(row,col)
    src_pixel_coords = pixel_coords + flow # new coordinate (y,x) in the source plane shifting by flow
    output_depth, mask = bilinear_sampler(src_depth, src_pixel_coords, "yx")

    return output_depth, mask
...
def bilinear_sampler(imgs, coords, mode="xy"):
    ...
    with tf.name_scope('image_sampling'):
        if mode == "xy":
            ...
            coords_y, coords_x = tf.split(coords, [1, 1], axis=3)
        ...
        y_max = tf.cast(tf.shape(imgs)[1] - 1, 'float32') * tf.ones_like(y0)
        x_max = tf.cast(tf.shape(imgs)[2] - 1, 'float32') * tf.ones_like(x0)
        zero = tf.zeros_like(x0, dtype='float32')
