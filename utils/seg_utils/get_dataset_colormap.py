...
def label_to_color_image(label, dataset=_CITYSCAPES):
  ...
  Raises:
    ValueError: If label is not of rank 2 or its value is larger than color
      map maximum entry.
  """
  if label.shape.as_list()[-1] != 1:
    raise ValueError('Expect 2-D input label or 4-D [B,h,w,1] input label. Got {}'.format(label.shape.as_list()))
  ...
  colormap = tf.convert_to_tensor(colormap, name="segmentation_colormap")
  indices = tf.reshape(tf.cast(label, tf.int32), [-1,1])
  seg_color = tf.reshape(tf.gather_nd(colormap, indices), label.shape.as_list()[:-1] + [3])
  return seg_color
