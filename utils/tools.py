...
def grayscale(inputs, mode="1"):
    """
    Args
    ====
    - inputs: input state. [B, h, w, c]
    - mode:
        | https://www.johndcook.com/blog/2009/08/24/algorithms-convert-color-grayscale/

            1) average method [default] : (R+G+B) / 3
    ...
    """
    assert inputs.shape.as_list()[-1] == 3
    if mode == '2' or "light" in mode:
        M = tf.reduce_max(inputs, axis=3)
        m = tf.reduce_min(inputs, axis=3)
        output = (M+m) / 2.
    elif mode == '3' or "lumi" in mode:
        R, G, B = tf.split(inputs, axis=3, num_or_size_splits=3)
        output = 0.21 * R + 0.72 * G + 0.07 * B
    else:
        output = tf.reduce_mean(inputs, axis=3)
...
def euler2mat(z, y, x):
  ...
  B = tf.shape(z)[0]
  N = 1
  z = tf.clip_by_value(z, -np.pi * tf.ones_like(z), np.pi * tf.ones_like(z))
  y = tf.clip_by_value(y, -np.pi * tf.ones_like(y), np.pi * tf.ones_like(y))
  x = tf.clip_by_value(x, -np.pi * tf.ones_like(x), np.pi * tf.ones_like(x))
...
def bilinear_sampler(imgs, coords):
  ...
  with tf.name_scope('image_sampling'):
    ...
    coords_x = tf.clip_by_value(coords_x, eps * tf.ones_like(coords_x), (x_max - eps) * tf.ones_like(coords_x))
    coords_y = tf.clip_by_value(coords_y, eps * tf.ones_like(coords_y), (y_max - eps) * tf.ones_like(coords_y))
    ...
    x0_safe = tf.clip_by_value(x0, tf.zeros_like(x0), x_max * tf.ones_like(x_max))
    y0_safe = tf.clip_by_value(y0, tf.zeros_like(y0), y_max * tf.ones_like(y_max))
    x1_safe = tf.clip_by_value(x1, tf.zeros_like(x1), x_max * tf.ones_like(x_max))
    y1_safe = tf.clip_by_value(y1, tf.zeros_like(y1), y_max * tf.ones_like(y_max))
