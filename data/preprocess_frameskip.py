...
def dump_example(n, is_training):
    ...
    for suffix in example['suffix']:
        # save image file
        if not args.skip_image:
            image_seq = concat_image_seq(example['image_seq'][suffix])
            dump_img_file = dump_dir + '/%s_%s.jpg' % (example['file_name'], suffix)
        ...
        if is_training:
            ...
            cx = intrinsics[0, 2]
            cy = intrinsics[1, 2]
            dump_cam_file = dump_dir + '/%s_%s_cam.txt' % (example['file_name'], suffix)
            with open(dump_cam_file, 'w') as f:
                f.write('%f,0.,%f,0.,%f,%f,0.,0.,1.\n' % (fx, cx, fy, cy))
                if 'pose' in example:
                    poses = example['pose'][suffix] # (rz,ry,rx,tx,ty,tz)
                    if poses is False:
                        print ("====>", dump_cam_file, image_seq.shape)
...
def main():
    ...
    if args.dataset_name == 'kitti_odom':
        from kitti.kitti_odom_loader_skip1 import kitti_odom_loader
