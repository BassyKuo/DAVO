...
class Net(object):
    def __init__(self,is_training, opt, pose_scale=0.01):
        self.is_training = is_training
        self.decay = np.float(0.95)
        self.epsilon = np.float(1e-5)
        self.batch_size = np.int(opt.batch_size) 
        self.pose_scale = pose_scale
        self.W = np.int(opt.img_width)
        self.H = np.int(opt.img_height)
    ...
    def build_pose_net3_dc(self, res18):
        print('Building Pose Decoder Model (decoupling)')
        with tf.variable_scope('pose_decoder', reuse=tf.AUTO_REUSE) as scope:
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('rotation'):
                    rot_conv1 = self._conv(res18_concat, 3, 256, 1, name='rot_conv1')
                    rot_conv2 = self._conv(rot_conv1, 3, 256, 1, name='rot_conv2')
                    rot_conv3 = slim.conv2d(rot_conv2, 3, 1, stride=1, scope='rot_conv3', activation_fn=None)
                    rot_final = tf.reduce_mean(rot_conv3, [1, 2], keepdims=True)
                    rot_final = tf.reshape(rot_final, [self.batch_size, 1, 3])
                with tf.variable_scope('translation'):
                    trans_conv1 = self._conv(res18_concat, 3, 256, 1, name='trans_conv1')
                    trans_conv2 = self._conv(trans_conv1, 3, 256, 1, name='trans_conv2')
                    trans_conv3 = slim.conv2d(trans_conv2, 3, 1, stride=1, scope='trans_conv3', activation_fn=None)
                    trans_final = tf.reduce_mean(trans_conv3, [1, 2], keepdims=True)
                    trans_final = tf.reshape(trans_final, [self.batch_size, 1, 3])
                pose_final = tf.concat([rot_final, trans_final], axis=-1)
